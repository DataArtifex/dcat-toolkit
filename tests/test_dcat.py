...
class TestResource:
    ...
    def test_add_landing_page(self):
        ...
        resource = dcat.Resource(id="resource-1")
        url = resource.add_landing_page("http://example.org/page")
        assert len(resource.landingPage) == 1
        assert "http://example.org/page" in resource.landingPage
    
    def test_add_modified_date_string(self):
    ...
    def test_add_modified_date_datetime(self):
    ...
    def test_add_release_date(self):
...
class TestDataset:
    ...
    def test_add_distribution_type_error(self):
        """Test that adding wrong type raises TypeError."""
        dataset = dcat.Dataset(id="dataset-1")
        with pytest.raises(TypeError):
            dataset.add_distribution("not a distribution")
    
    def test_add_theme_string(self):
        """Test adding a theme from string."""
        dataset = dcat.Dataset(id="dataset-1")
        theme = dataset.add_theme("http://example.org/themes/science")
...
class TestCatalog:
    ...
    def test_add_theme_taxonomy(self):
        """Test adding a theme taxonomy."""
        catalog = dcat.Catalog(id="catalog-1")
        taxonomy = catalog.add_theme_taxonomy("http://example.org/taxonomy")
...
class TestDistribution:
    ...
    def test_add_download_url_string(self):
        """Test adding download URL from string."""
        dist = dcat.Distribution(id="dist-1")
        url = dist.add_download_url("http://example.org/data.csv")
    ...
    def test_add_media_type(self):
        ...
        dist = dcat.Distribution(id="dist-1")
        media = dist.add_media_type("text/csv")
...
class TestDataService:
    ...
    def test_add_endpoint_url(self):
        """Test adding endpoint URL."""
        service = dcat.DataService(id="service-1")
        url = service.add_endpoint_url("http://api.example.org/v1")
        assert len(service.endpointURL) == 1
        assert "http://api.example.org/v1" in service.endpointURL
    
    def test_add_served_dataset(self):
...
class TestRdfSerialization:
    ...
    def test_catalog_with_dataset_to_rdf(self):
...
class TestIntegration:
    ...
    def test_catalog_with_service(self):
    ...
    def test_complete_rdf_serialization(self):
