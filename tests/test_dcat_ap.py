...
class TestDataset:
    ...
    def test_add_applicable_legislation(self):
        """Test adding applicable legislation (HVD)."""
        dataset = dcat_ap.Dataset(id="dataset-1")
        hvd_reg = dataset.add_applicable_legislation(
            "http://data.europa.eu/eli/reg_impl/2023/138/oj"
        )
    ...
    def test_set_access_rights(self):
        ...
        dataset = dcat_ap.Dataset(id="dataset-1")
        access = dataset.set_access_rights(
    ...
    def test_add_provenance(self):
        ...
        dataset = dcat_ap.Dataset(id="dataset-1")
        prov = dataset.add_provenance("http://example.org/provenance/1")
        assert len(dataset.provenance) == 1
    
    def test_add_qualified_attribution(self):
        """Test adding qualified attribution."""
        dataset = dcat_ap.Dataset(id="dataset-1")
        attr = dataset.add_qualified_attribution("http://example.org/attribution/1")
        assert len(dataset.qualifiedAttribution) == 1
    
    def test_add_other_identifier(self):
        """Test adding ADMS identifier."""
        dataset = dcat_ap.Dataset(id="dataset-1")
        ident = dataset.add_other_identifier("http://example.org/identifier/alt-1")
        assert len(dataset.otherIdentifier) == 1
    
    def test_dataset_to_rdf(self):
...
class TestDistribution:
    ...
    def test_add_applicable_legislation(self):
        """Test adding applicable legislation (HVD)."""
        dist = dcat_ap.Distribution(id="dist-1")
        hvd_reg = dist.add_applicable_legislation(
            "http://data.europa.eu/eli/reg_impl/2023/138/oj"
        )
...
class TestDataService:
    ...
    def test_add_applicable_legislation(self):
        """Test adding applicable legislation (HVD)."""
        service = dcat_ap.DataService(id="service-1")
        hvd_reg = service.add_applicable_legislation(
            "http://data.europa.eu/eli/reg_impl/2023/138/oj"
        )
...
class TestDatasetSeries:
    ...
    def test_create_dataset_series(self):
    ...
    def test_add_applicable_legislation(self):
        """Test adding applicable legislation (HVD)."""
        series = dcat_ap.DatasetSeries(id="series-1")
        hvd_reg = series.add_applicable_legislation(
            "http://data.europa.eu/eli/reg_impl/2023/138/oj"
        )
    ...
    def test_set_access_rights(self):
        ...
        series = dcat_ap.DatasetSeries(id="series-1")
        access = series.set_access_rights(
            "http://publications.europa.eu/resource/authority/access-right/PUBLIC"
        )
...
class TestCatalogRecord:
    ...
    def test_add_applicable_legislation(self):
        """Test adding applicable legislation (HVD)."""
        record = dcat_ap.CatalogRecord(id="record-1")
        hvd_reg = record.add_applicable_legislation(
            "http://data.europa.eu/eli/reg_impl/2023/138/oj"
        )
...
class TestControlledVocabularies:
    ...
    def test_controlled_vocabularies_exist(self):
    ...
    def test_hvd_categories_exist(self):
...
class TestHVDCompliance:
    ...
    def test_hvd_distribution(self):
        ...
        # Add HVD regulation
        dist.add_applicable_legislation(
            "http://data.europa.eu/eli/reg_impl/2023/138/oj"
        )
...
class TestRdfSerialization:
    ...
    def test_round_trip(self):
        ...
        # Deserialize
        subject = URIRef(str(dcat_ap.DCAT) + "dataset-1")
...
class TestIntegration:
    ...
    def test_complete_catalog_structure(self):
    ...
    def test_multiple_rdf_serialization(self):
        ...
        catalog_uri = URIRef(str(dcat_ap.DCAT) + "catalog-1")
        dataset_uri = URIRef(str(dcat_ap.DCAT) + "dataset-1")
        
        assert (catalog_uri, RDF.type, dcat_ap.DCAT.Catalog) in graph
        assert (dataset_uri, RDF.type, dcat_ap.DCAT.Dataset) in graph
